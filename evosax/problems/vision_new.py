...
class VisionFitnessNew(object):
    def __init__(
        ...
        self.data = get_array_data(self.task_name, self.test)
        self.dataloader = BatchLoader(*self.data, batch_size=self.batch_size, test=self.test)
    ...
    def set_apply_fn(self, map_dict, network):
        ...
        if self.test:
            ...
            self.rollout_pop = jax.vmap(self.rollout_ffw_new, in_axes=(None, map_dict))
        ...
        if self.n_devices > 1:
            ...
            if self.test:
                ...
                self.rollout = jax.jit(self.rollout_vmap2)

    def rollout_vmap2(
        self, rng_input: chex.PRNGKey, network_params: chex.ArrayTree
    ):
        """Vectorize rollout. Reshape output correctly."""
        y_pred, y = self.rollout_pop(rng_input, network_params)
        # y_pred_re = y_pred.reshape(-1, 1)
        return y_pred, y[0]
    def rollout_vmap(
        self, rng_input: chex.PRNGKey, network_params: chex.ArrayTree
    ):
    ...
    def rollout_ffw(
        ...
        if 'params' in network_params.keys():
